...
def score_keys(cipher_text, frequency_dict, scores, sensitivity):
    i = 0 
    while i < 256: # iterate through ASCII values of all printable characters
        ...
        while k < len(cipher_text): # XOR value of each character in encrypted string with current test value i
            ...
            if (c_xor_i < 32) or (c_xor_i > 126):
                scores[chr(i)] -= 1
                k += 1
            elif ((c_xor_i >= 33) and (c_xor_i <= 64)) or ((c_xor_i >= 91) and (c_xor_i <= 96)) or (c_xor_i > 122):
                scores[chr(i)] -= 1
                k += 1
            elif ((c_xor_i >= 65) and (c_xor_i <= 90)) or ((c_xor_i >= 97) and (c_xor_i <= 122)) or (c_xor_i == 32):
    ...
    """
    # If one of the key characters generated doesn't produce the correct plaintext when used, dig in
    # by printing out the list of key_scores to grab the next highest value and replace
